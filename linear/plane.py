...
class Plane(object):
    ...
    def is_parallel_to(self, p2):
        ...
        v2 = p2.normal_vector
        if v1.testParallel(v2):
            return True
        else:
            return False

    def is_same_plane(self, p2):
        if self.is_parallel_to(p2):
            x1, y1, z1 = self.basepoint.coordinates
            x2, y2, z2 = p2.basepoint.coordinates
            v = Vector([x1 - x2, y1 - y2, z1 - z2])
            d = round(v.dot(self.normal_vector), 3)
            print(d)
            if d == 0:
                return True
            else:
